 ...
 *   voice_id, redirect_to: '/dashboard' ('/v2' when V2_DEFAULT_ON=true) }
 *
...
${extracted.openingHours && Object.keys(extracted.openingHours).length ? Object.entries(extracted.openingHours).map(([day, h]) => h.closed ? `${day}: Closed` : `${day}: ${h.open || '?'} - ${h.close || '?'}`).join('\n') : 'Not specified'}`,
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      // setup-launch only enables V2 under the same flag; classic otherwise.
      redirect_to: process.env.V2_DEFAULT_ON === 'true' ? '/v2' : '/dashboard',
    }),
  };
};
