...
export type AgencyEventType =
  ...
  // SaaS V2 dashboard surface (wave-2 pages — telemetry only, never client-facing)
...
// ── SaaS V2 telemetry schemas (wave-2 pages) ──────────────────────────────
//
// These are emitted by /v2/* pages and their backing functions. They use
// `client_id` as the workspace_id (the column name is kept for substrate
// compatibility — see saas-v2-toggle.ts for the rationale). Strict allowlist
// keeps the events table from becoming a fire-hose; never log raw prompts,
// transcripts, or PII.
...
const EVENT_SCHEMAS = {
  ...
  // SaaS V2 telemetry (wave-2 pages)
...
const CLIENT_VISIBLE_FIELDS: Record<AgencyEventType, ReadonlyArray<string>> = {
  ...
  subscription_changed:             [],
  // SaaS V2 telemetry (wave-2 pages) — strictly internal observability.
  saas_v2_agent_summary_rendered:   [],
  saas_v2_agent_stress_test_run:    [],
  saas_v2_agent_suggest_edits:      [],
};
