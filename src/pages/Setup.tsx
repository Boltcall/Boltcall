...
const SetupInner: React.FC = () => {
  ...
  const { updateBusinessProfile, updateAgentConfig, updateCallFlow } =
    useSetupStore();
  ...
  const [voiceId, setVoiceId] = useState<
    (typeof VOICE_OPTIONS)[number]['value']
  ...
  async function enqueueAssistant(content: string) {
    ...
    setMessages((prev) => [
      ...prev,
      { id, role: 'assistant', content, displayed: 0 },
    ]);
  ...
  async function handleGoogleContinue() {
    try {
      ...
      if (
        ...
        authError.message === 'OAuth redirect initiated'
      )
        return;
      setError(
        authError instanceof Error
          ...
          <aside className="flex flex-col justify-between rounded-[28px] border border-amber-200/70 bg-[radial-gradient(circle_at_top,#fff4bf_0%,#fff8df_34%,#fffdf6_100%)] p-6 shadow-[0_24px_80px_rgba(251,191,36,0.12)]">
            <div>
              ...
              <p className="mt-4 text-sm leading-6 text-zinc-700">
                We’ll keep the first steps tight: business name, industry, and
                the core voice preferences that shape how the agent answers your
                leads.
              </p>
            </div>
            ...
            <div className="mt-8 space-y-3 text-sm text-zinc-700">
              <div className="flex items-start gap-3 rounded-2xl bg-white/80 p-3 shadow-sm">
                ...
                <div>
                  <div className="font-medium text-zinc-900">
                    Feels like a walkthrough
                  </div>
                  <div className="mt-1 text-zinc-600">
                    Structured fields appear inside the conversation, not in a
                    generic form wizard.
                  </div>
                </div>
              </div>
              <div className="flex items-start gap-3 rounded-2xl bg-white/80 p-3 shadow-sm">
                ...
                <div>
                  <div className="font-medium text-zinc-900">
                    Google auth comes after momentum
                  </div>
                  <div className="mt-1 text-zinc-600">
                    You answer the first two setup turns, then save and continue
                    straight into your live agent walkthrough.
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-semibold text-zinc-950">
                    Boltcall setup agent
                  </div>
                  <div className="text-xs text-zinc-500">
                    {step === 'business' && 'Step 1 of 2 · business basics'}
                    {step === 'preferences' &&
                      'Step 2 of 2 · agent preferences'}
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <FieldGroup label="Voice" htmlFor="voiceId">
                      <select
                        ...
                        value={voiceId}
                        onChange={(event) =>
                          setVoiceId(event.target.value as typeof voiceId)
                        }
                        className="flex h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground shadow-sm shadow-black/5 outline-none focus:border-ring focus:ring-[3px] focus:ring-ring/20"
                      >
