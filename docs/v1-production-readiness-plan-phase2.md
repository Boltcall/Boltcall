...
- [ ] Test call to a Boltcall Retell number (e.g. `+972 3 376 1315` or `+1 361 304 4585`) → confirm a row lands in `retell_calls` after ≥8s answered call — needs Noam's phone. Code path proven via HMAC probe (2026-07-29); this is end-to-end validation, not a bug hunt.
